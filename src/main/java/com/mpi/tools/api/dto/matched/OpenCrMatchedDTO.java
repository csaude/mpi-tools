...
public class OpenCrMatchedDTO implements Matched{
	...
	public Matched goNextPage(MatchedPatientFeignClient client, MatchIssueFeignClient matchIssueClient, MatchConfig config, MatchIssueService matchIssueService) {
		...
		for (LinkDTO nextPage : this.getLink()) {
			if (nextPage.getRelation().equals("next")) {
				if (!nextPage.getUrl().isEmpty()) {
					...
					return null;//client.getPatientNextPage(nextURI);
				}
			}
		}
